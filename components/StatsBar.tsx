...
export default function StatsBar({ orders }: { orders: Order[] }) {
  ...
  const deltaStyle = (type: 'up' | 'down' | 'warn'): React.CSSProperties => ({
      ...
      {stats.map(s => (
        <div key={s.label} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
            <span style={{ fontSize: '11px', fontWeight: 500, color: '#9299A8', letterSpacing: '0.02em' }}>{s.label}</span>
            <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: s.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <s.icon size={14} strokeWidth={1.4} style={{ color: s.iconColor }} />
            </div>
          </div>
          <div style={{ fontSize: '26px', fontWeight: 600, color: s.valueColor, letterSpacing: '-0.6px', lineHeight: 1, marginBottom: '8px', fontVariantNumeric: 'tabular-nums' }}>
            {s.value}
          </div>
          {s.delta && <span style={deltaStyle(s.delta.type)}>{s.delta.text}</span>}
        </div>
      ))}
    </div>
  )
